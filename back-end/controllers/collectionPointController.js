...
exports.getCollectionPointById = async (req, res) => {
	try {
		const cleanedId = req.params.id.replace(/[^\d]/g, ""); // Supprime tout sauf les chiffres
		const id = parseInt(cleanedId, 10);
		...
		res
			.status(500)
			.json({ message: "Erreur lors de la récupération du point de collecte" });
	}
};
...
exports.createCollectionPoint = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ message: "Erreur lors de la création du point de collecte" });
	}
};
...
exports.updateCollectionPoint = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ message: "Erreur lors de la mise à jour du point de collecte" });
	}
};
...
exports.deleteCollectionPoint = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ message: "Erreur lors de la suppression du point de collecte" });
	}
};
